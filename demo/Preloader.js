/**
 * @summary Web-page resources Preloader and their smart-activator, display loader as download bar with %, speed, current/total bytes etc.
 * @description Web-page resources Preloader and their smart-activator, display loader as download bar with %, speed, current/total bytes etc.
 * Класс Preloader - предзагрузчик ресурсов веб-страницы, показывает полоску процесса предзагрузки, скорость и прочие детали.
 ...
 * .
 * Для указания того, что ресурс url нужно только кешировать, используется dataset-атрибут: data-cache=url
 * Для указания того, что ресурс url нужно активировать после кеширования, необходимо переименовать содержащий url атрибут
 * в его соответствующий dataset-атрибут:
 ...
 * После кеширования и активации ресурсов происходит удаление dataset-атрибутов, чем можно воспользоваться, например,
 * для создания CSS-анимации плавного проявления фона после его активации:
 * <pre>
 * div {
 *      opacity: 1;
 ...
 *
 * Установка:
 * 1. Скопировать файл getFilesSize.php в папку 'php' (изменить путь к скрипту можно в {@link _getFilesSize}.
 * Это скрипт php, который принимает массив ссылок на файлы и возвращает массив их размеров.
 * 2. Скопировать файл Preloader.js
 * Использование:
 ...
 * 4. Создать экземпляр Preloader без параметров или с указанием callback (тип function).
 * 5. При необходимости, тонкую настройку можно произвести с помощью {@link config}
 * 7. Запуск процесса кеширования с помощью {@link start}.
 * Т.к. Preloader использует DOM, то вызов {@link start} должен быть не ранее события DOMContentLoaded
 * Расположение ресурсов поддерживаются в соответствии с политикой same-origin, реализация CORS находится в стадии разработки
 * .
 * @example
 * // 1.
 * // Кеширование и последующая активация ресурсов из всего документа,
 * // с показом loader в виде полоски загрузки с %, скоростью и т.д.
 * window.onload = () => (new Preloader()).start();
 *
 * // 2.
 * // то же самое, но с callback
 * window.onload = () => {
 ...
 *
 * // 3.
 * // кеширование и активация ссылок, найденных в пределах елемента с селектором
 * // '.gallery', указание callback, загрузка фоновых изображений сразу после
 * // применения внешнего набора правил CSS (внешний css содержит: body{color:red})
 ...
 *
 * @param {Function|undefined} [callback] функция, вызываемая при успешном кешировании всех ссылок.
 * Устанавливается до вызова {@link start} в {@link Preloader.constructor} или в {@link config}
 * .
 * @see Preloader.constructor
 * @TODO реализовать i18n в пределах t9n
 * @TODO перевести комментарии на английский
 * @TODO реализовать retry при сетевой ошибке во время загрузки
 * @TODO детальнее провести дифференцирование ошибок при загрузке ресурса
 * @TODO учесть случай, когда условие проверки CSS никогда не станет истинным (условие проверки применения к DOM внешнего набора CSS правил)
 * @TODO добавить front-end альтернативу back-end реализации определения размеров файлов
 * @TODO добавить дифференцирование ошибок proxy
 * @TODO добавить поддержку CORS для использования нелокальных ресурсов
 * @TODO добавить опционально режим загрузки, при котором ресурсы, указанные в activateAfterCssRulesCondition, будут загружаться первыми
 * @TODO шаблонизировать внешний вид для возможности добавлять шаблоны лоадеров
 * @TODO implement urls de-duplication using the `set` collection
 * @TODO rendering artifacts in firefox? It looks like they gone
 * @translation
...
window.Preloader = class Preloader {
    ...
    /**
     * Запуск Preloader
     * @throws {Error} при отсутствии callback
     */
    start() {
